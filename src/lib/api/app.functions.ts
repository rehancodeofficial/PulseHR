...
// Leave Balances
// ------------------------------------
export async function getLeaveBalancesFn(employeeId?: string) {
  return (await LeavesAPI.getBalances(employeeId)) as any;
}

// ------------------------------------
// Payroll
// ------------------------------------
export async function getPayrollFn(params?: any) {
  return (await PayrollAPI.getAll(params)) as any;
}

export async function getMyPayslipsFn() {
  return (await PayrollAPI.getMy()) as any;
}

export async function generatePayrollFn({
  data,
}: {
  data: { month: number; year: number; departmentId?: string; deductionPercent?: number };
}) {
  return (await PayrollAPI.generate(data)) as any;
}

export async function updatePayrollFn({
  data,
}: {
  data: { id: string; updates: any };
}) {
  return (await PayrollAPI.update(data.id, data.updates)) as any;
}
